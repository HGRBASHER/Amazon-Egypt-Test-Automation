...
public class ProductDetailsPage extends BasePage {
    ...
    private final By priceWhole = By.xpath("//div[@id='corePrice_feature_div']//span[@class='a-price-whole']");
    ...
    public double getProductPrice() {
            ...
            try {
                ...
                try {
                    ...
                    if (rawText != null && !rawText.isEmpty()) {
                        String cleanPrice = rawText.replaceAll("\n", ".").replaceAll("[^0-9.]", "");
        ...
        // الـ Fallback الآمن والمعدل عشان يلقط الكسور العشريّة بدقة لو الـ XPaths اللي فوق منجحتش
        try {
    ...
    public boolean addToCart() {
        ...
        try {
            shortWait.until(ExpectedConditions.elementToBeClickable(addToCartButton)).click();
            try {
                shortWait.until(ExpectedConditions.elementToBeClickable(warrantyCloseButton)).click();
            } catch (Exception e) {
                try {
    ...
    public boolean isAvailabilityStatusDisplayed() {
        try {
            return wait.until(ExpectedConditions.visibilityOfElementLocated(availabilityStatus)).isDisplayed();
        } catch (Exception e) {
            return false;
        }
    }
    public boolean isProductImagesDisplayed() {
        try {
            return wait.until(ExpectedConditions.visibilityOfElementLocated(productImagesContainer)).isDisplayed();
        } catch (Exception e) {
            return false;
        }
    }
    public boolean isRatingSectionDisplayed() {
        try {
            return wait.until(ExpectedConditions.visibilityOfElementLocated(ratingSection)).isDisplayed();
        } catch (Exception e) {
            return false;
        }
    }
